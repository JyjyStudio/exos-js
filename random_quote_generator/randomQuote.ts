...
const quoteRender = document.getElementById('quote') as HTMLElement;
const authorRender = document.getElementById('author') as HTMLElement;
const nextQuote = document.getElementById('nextQuote') as HTMLElement;
let randomId = 0;
let lastId = 0;

const randomIndex = (quote = quotes.length) => {    
    return Math.floor((Math.random()) * (quotes.length -1));  // return number between 0 & quotes.length
}

nextQuote.addEventListener('click', ()=>{
    // to avoid having the same quote twice:
    do { 
        randomId = randomIndex();
    } while (randomId == lastId); 
    
    quoteRender.textContent = quotes[randomId][0]
    authorRender.textContent = quotes[randomId][1]
